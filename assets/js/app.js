    ...
    function renderProfile() {
      ...
      qs('[data-profile-overview]').innerHTML = `
        <div class="profile-hero-mini">
          ...
          <div><p class="eyebrow">Perfil activo</p><h2>${escapeHtml(profile.name)}</h2><p>${escapeHtml(profile.bio || role.purpose)}</p></div>
        </div>
        <div class="profile-panel-stack">
          <div class="tag-cloud"><span class="tag">${escapeHtml(role.label)}</span><span class="tag">${escapeHtml(profile.city)}</span><span class="tag">${escapeHtml(profile.code || 'Cuenta creada')}</span></div>
          ...
          <div class="profile-summary-grid"><article class="mini-card"><strong>Acción principal</strong><span>${escapeHtml(role.action)}</span></article><article class="mini-card"><strong>Sentido del rol</strong><span>${escapeHtml(role.purpose)}</span></article><article class="mini-card"><strong>Permisos</strong><span>${state.acceptedLegal ? 'Autorizaciones registradas' : 'Autorizaciones pendientes'}</span></article><article class="mini-card"><strong>Última cápsula</strong><span>${escapeHtml(activeArtist().track)} · ${escapeHtml(activeArtist().name)}</span></article></div>
          <div class="profile-activity-strip"><span class="status-chip">${escapeHtml(role.action)}</span><span class="status-chip">${escapeHtml(role.profileAxis)}</span><span class="status-chip">Cali</span></div>
    ...
    function roleWorkspace(profile) {
      ...
      const radar = profile.role === 'company' ? `<div class="company-radar-grid">${runtimeArtists().slice(0, 5).map(artist => `<article class="list-item"><span class="list-cover tone-${escapeHtml(artist.tone || 'sunset')}">${imgMarkup(artist.cover, artist.track, artist.symbol)}</span><div class="list-item-content"><strong>${escapeHtml(artist.name)}</strong><span>${artist.match}% afinidad · ${escapeHtml(artist.scene)}</span></div><button class="soft-button ${state.saved.includes(artist.id) ? 'is-active' : ''}" type="button" data-save-artist="${escapeHtml(artist.id)}">${state.saved.includes(artist.id) ? 'En radar' : 'Radar'}</button></article>`).join('')}</div>` : '';
      const playlists = ['user', 'curator', 'ambassador'].includes(profile.role) ? `<div class="playlist-grid">${playlistCards}</div>` : '';
      const features = roleFeatureCards(workspace.features);
      const actions = roleActionButtons(workspace.actions);
      return `<div class="panel-head"><div><p class="eyebrow">${escapeHtml(workspace.eyebrow || 'Rol KORA')}</p><h2>${escapeHtml(workspace.title || role.label)}</h2></div><span class="counter-pill">${escapeHtml(workspace.badge || role.action)}</span></div><p>${escapeHtml(workspace.intro || role.purpose)}</p>${radar}<div class="role-action-grid">${features}</div>${playlists}${actions ? `<div class="role-work-actions">${actions}</div>` : ''}`;
    }
    ...
    function privacyCards(profile) {
      ...
      const base = [
        ...
        { title: 'Control de acceso por roles', body: 'Cada rol opera como una forma de participación: descubrir, recomendar, contextualizar, publicar o analizar.' },
      ...
      (role.privacy || []).forEach(item => base.push({ title: item[0], body: item[1] }));
      return base;
    }
    ...
    function showLegalGate(force = false) {
      ...
      qs('#consent-title').textContent = roleLegal.title;
      const lead = qs('[data-consent-role-lead]');
      if (lead) lead.textContent = roleLegal.lead;
      const copyMap = { terms: roleLegal.terms, privacy: roleLegal.privacy, roleData: roleLegal.roleData };
      Object.keys(copyMap).forEach(name => {
        const text = qs(`[data-legal-copy="${name}"]`, form);
        if (text) text.innerHTML = escapeHtml(copyMap[name]);
      });
    ...
    function remindViewGuideLater(view) {
      if (!state.viewGuideSnoozed) state.viewGuideSnoozed = {};
      const key = viewGuideKey(view);
      state.viewGuideSnoozed[key] = Date.now() + 60000;
      saveState();
      closeModal();
      toast('Te lo recordaremos más tarde.');
      setTimeout(() => maybeShowViewGuide(view), 61000);
    }
    ...
    function showWelcomeOnboarding(force = false) {
      ...
      if (!force && Date.now() < (state.onboardingSnoozedUntil || 0)) return;
      showModal(`
        <div class="onboarding-hero">
          <p class="eyebrow">Bienvenida narrativa</p>
        ...
        <div class="onboarding-summary">En KORA puedo descubrir, guardar, comentar, compartir y participar en una comunidad musical local.</div>
        <div class="onboarding-actions">
          <button class="primary-button" type="button" data-finish-onboarding>Entrar a KORA</button>
          <button class="soft-button" type="button" data-remind-onboarding>Recordármelo luego</button>
        </div>
      `, 'onboarding-card');
    }
    
    function remindOnboardingLater() {
      state.onboardingSnoozedUntil = Date.now() + 60000;
      saveState();
      closeModal();
      toast('Te recordaremos la bienvenida más tarde.');
      setTimeout(() => showWelcomeOnboarding(), 61000);
    }
